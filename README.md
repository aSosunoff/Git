...
> заменяет коммит не создавая новый
...
> запрещает комит перемоткой. и делает комит слияния (создаёт новый комит со слиянием)
...
## rebase - _перенос комитов. перебазирование коммитов_

```
означает что возмёт изменения из указанного коммита и переместит с этими изменениями в верх сделая при этом предком расхождения тот коммит который указали. как минимум получаем разницу в истории коммитов. картинка со слияниями будет сложнее чем если использовать перебазирование. но перебазирование лучше делать с приватными ветками так как публичные затруднят работу остальным сотрудникам.

+ упрощает историю
- только с приватными ветками
- возможны ошибки в коммитах
```

`{commit}`

> перенос своих комитов выше комита который был указан но с изменениями которые были в указанном коммите. тоже самое можно сделать через `git merge master`. 

`--abort`

> отмена переноса с возвратом HEAD

`--quit`

> отмена переноса без возврата HEAD

`--continue`

> продоление переноса если был конфликт после разрешения конфликта

`-x '...' master`

> запуск тестов после каждого перебазированного комита

`--onto {branch1} {branch2}`

> перенос текущей ветки начиная от ветки 1 с ветки 2