...
```
означает что возмёт изменения из указанного коммита и переместит с этими изменениями в верх сделая при этом предком расхождения тот коммит который указали.
```

`{commit}`

> перенос своих комитов выше комита который был указан но с изменениями которые были в указанном коммите. тоже самое можно сделать через `git merge master`. как минимум получаем разницу в истории коммитов. картинка со слияниями будет сложнее чем если использовать перебазирование.
